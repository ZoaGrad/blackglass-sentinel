...
STATUS_PATH = Path(__file__).with_name("sentinel_status.json")
...
from output.god_mode_hardened import (
    GodModeValidationError,
    _write_audit_event,
...
def _reject_legacy_god_mode_env() -> bool:
    """Reject the retired env-var override path and record the attempt."""
    if os.getenv("SENTINEL_GOD_MODE", "").strip().lower() != "true":
        return False

    logger.error(
        "ACCESS DENIED: SENTINEL_GOD_MODE override retired. "
        "Use SENTINEL_GOD_MODE_TOKEN instead."
    )
    _write_audit_event(
        "GOD_MODE_REJECTED",
        {
            "reason": "legacy_env_override_retired",
            "source": "SENTINEL_GOD_MODE",
        },
    )
    return True


def _write_status_snapshot(status_data: dict) -> None:
    """Persist the latest Sentinel fatigue status as local runtime state."""
    STATUS_PATH.write_text(json.dumps(status_data) + "\n", encoding="utf-8")


def check_god_mode(request: Optional[dict] = None) -> dict:
    """Validate the signed God Mode token and return override metadata."""
    if _reject_legacy_god_mode_env():
        return {
            "active": False,
            "reason": "legacy_env_override_retired",
            "operator": None,
    ...
    request = request or {}
    token = request.get("god_mode_token") or os.getenv("SENTINEL_GOD_MODE_TOKEN", "").strip()
    if not token:
        return {
            "active": False,
            "reason": None,
    ...
    secret = os.getenv("SENTINEL_GOD_MODE_SECRET", "").strip()
    if not secret:
        logger.error(
            "GOD_MODE_REJECTED: SENTINEL_GOD_MODE_SECRET missing for token validation."
        )
        _write_audit_event(
            "GOD_MODE_REJECTED",
            {
    ...
    try:
        operator, locked_hour, expires_at = validate_god_mode_token(token, secret)
    except GodModeValidationError as exc:
        reason = str(exc)
        logger.error("GOD_MODE_REJECTED: %s", reason)
        _write_audit_event(
            "GOD_MODE_REJECTED",
            {
                "reason": reason,
    ...
    expires_utc = datetime.fromtimestamp(expires_at, tz=timezone.utc).isoformat()
    logger.warning(
        "GOD_MODE_ACTIVE: operator=%s, locked_hour=%s, expires=%s",
        operator,
        locked_hour,
        expires_utc,
    )
...
@mcp.tool()
def assess_human_cost(god_mode_token: Optional[str] = None) -> str:
    """
    ...
    god_mode = check_god_mode({"god_mode_token": god_mode_token} if god_mode_token else None)
    current_hour = god_mode["locked_hour"] if god_mode["active"] else denver_now.hour
    ...
    status_data = {
        ...
        "god_mode_operator": god_mode["operator"],
        "god_mode_expires": god_mode["expires_utc"],
        "god_mode_reason": god_mode["reason"],
    }
    _write_status_snapshot(status_data)
